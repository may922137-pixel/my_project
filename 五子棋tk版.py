...
class GobangApp:
    def __init__(self, root):
        ...
        # 状态标签
        self.status_label = tk.Label(top_frame, text="黑棋 (●) 的回合", font=("Arial", 16))
        self.status_label.pack(side=tk.LEFT, padx=20)

        # "新游戏" 按钮
        self.reset_button = tk.Button(top_frame, text="新游戏", font=("Arial", 14), command=self.reset_game)
        self.reset_button.pack(side=tk.RIGHT, padx=20)

        # 棋盘 Canvas
        self.canvas = tk.Canvas(self.root, width=canvas_dim, height=canvas_dim, bg=COLOR_BOARD)
    ...
    def draw_board(self):
        ...
        for i in range(BOARD_SIZE):
            ...
            # (x1, y1, x2, y2)
            # 横线
            self.canvas.create_line(PADDING, pos, max_coord, pos, fill=COLOR_LINE, tags="grid_line")
            # 竖线
            self.canvas.create_line(pos, PADDING, pos, max_coord, fill=COLOR_LINE, tags="grid_line")
            
        # 绘制标记点 (天元和星)
        star_points = [
            (3, 3), (11, 3), (3, 11), (11, 11),  # 四个角
            (7, 7)  # 天元
        ]
    ...
    def reset_game(self):
        ...
        """
        # 1. 重置逻辑棋盘
        self.board_logic = [[EMPTY_SLOT for _ in range(BOARD_SIZE)] for _ in range(BOARD_SIZE)]
    ...
    def make_move(self, row, col):
        ...
        if self.check_win(row, col, self.current_player):
            self.game_over = True
            winner_name = "黑棋 (●)" if self.current_player == PLAYER_BLACK else "白棋 (○)"
    ...
    def draw_piece(self, row, col, player):
        ...
        self.canvas.create_oval(x0, y0, x1, y1, fill=color, outline=outline_color, width=1, tags="piece")
    ...
    def check_win(self, r, c, player):
        ...
        for dr, dc in directions:
            ...
            for i in range(1, 5): 
                nr, nc = r + dr * i, c + dc * i
                if 0 <= nr < BOARD_SIZE and 0 <= nc < BOARD_SIZE and self.board_logic[nr][nc] == player:
                    count += 1
                else:
                    break
